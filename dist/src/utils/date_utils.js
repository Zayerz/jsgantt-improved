...
export const getMinDate = function (pList, pFormat, pMinDate) {
    ...
    if (pFormat == 'day') {
        vDate.setDate(vDate.getDate()); // Se quita el - 1 para que no agregue una semana más atrás
        while (vDate.getDay() % 7 != 1) vDate.setDate(vDate.getDate() - 1);
    } else if (pFormat == 'week') {
        vDate.setDate(vDate.getDate()); // Se quita el - 1 para que no agregue una semana más atrás
        while (vDate.getDay() % 7 != 1) vDate.setDate(vDate.getDate() - 1);
    } else if (pFormat == 'month') {
        vDate.setDate(vDate.getDate()); // Se quita el - 15 para que no agregue un mes más atrás
        while (vDate.getDate() > 1) vDate.setDate(vDate.getDate() - 1);
    } else if (pFormat == 'quarter') {
        vDate.setDate(vDate.getDate()); // Se quita el - 31 para que no agregue un mes más atrás
...
export const getMaxDate = function (pList, pFormat, pMaxDate) {
    ...
    if (pFormat == 'day') {
        vDate.setDate(vDate.getDate()); // Se quita el + 1 para que no agregue una semana más adelante
